...
(* Remove K refl to a term. *)
(* Fixpoint computeK (t : term) {struct t} : term := *)
(*   match t with *)
(*   | tCast t1 ck t2 => *)
(*     let t1' := computeK t1 in *)
(*     let t2' := computeK t2 in *)
(*     tCast t1' ck t2' *)
(*   | tProd n A B => *)
(*     let A' := computeK A in *)
(*     let B' := computeK B in *)
...
(*   | tApp f args => *)
(*     let f' := computeK f in *)
(*     let args' := map computeK args in *)
(*     tApp f' args' *)
(*   | tCase indn T t brs => *)
(*     let T' := computeK T in *)
(*     let t' := computeK T in *)
(*     let brs' := map (on_snd computeK) brs in *)
(*     tCase indn T' t' brs' *)
(*   | _ => t *)
(*   end. *)

(* Definition hnfK (t : term) : typing_result term := *)
(*   rt <- hnf_stack (Datatypes.fst Σ) [] t ;; *)
(*   ret (zip rt). *)

(* Definition tc_vrev2 : typing_result term := *)
(*   Eval native_compute in *)
(*   match tc_vrev with *)
(*   | FinalTranslation.Success _ t => hnfK t *)
(*   | _ => ret (tRel 0) *)
(*   end. *)

(* Make Definition coq_vrev2 := *)
(*   ltac:( *)
(*     let t := eval lazy in *)
(*              (match tc_vrev2 with *)
(*               | Checked t => t *)
